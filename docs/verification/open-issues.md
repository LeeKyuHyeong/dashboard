...
| O-005 | CLOSED | Test | `ProjectController` 의 `visible` 필터(숨긴 카드를 목록·상세 양쪽에서 차단)에 자동 테스트가 없다 | 09-17 작업에서 코드 라인(`ProjectController.java:21`·`:30`)과 운영 조회로만 확인했다 | — | 2026-09-25 | **2026-09-26 닫힘** — `ProjectVisibilityTest` 6건 신규(@DataJpaTest, 진짜 H2 + 진짜 파생 쿼리). 가드를 일부러 부수어 4건이 실패하는 것까지 확인. R-012 수동 → 자동 승격, R-013·R-014 신규. records/2026-09-26_project-visibility-test |
